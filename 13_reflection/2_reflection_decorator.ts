...
class Idol {
   ...
   * 이 후에 파라미터 검증값이나 검증 대상이 추가되면, 단순히 데코레이터를 조작하는 것으로
   * 해결할 수 있게 된다.
   */
  @ValidateMethod    
  // style에는 '신나게' | '열정적으로'
  ...
  @RestrictParamValue([1,2,3]) ranking: number) {
    /**
     * 1.
     * 메서드의 파라미터 type을 union 타입으로 지정해도 런타임 상에서는 JS로 동작하므로 그냥 string 타입이 된다.
     * 그래서 런타임 상에서 파라미터를 검증하기 위해 if문을 작성하게 된다.
     * 하지만 이러한 형태는 파라미터로 받아야 할 값이 변경되거나 검증해야 할 파라미터가 생길 때마다
     * if문을 계속 작성해야 한다.
     * 이는 결과적으로 가독성 뿐만 아니라 확장성, 유지보수성을 해치게 된다.
     */
